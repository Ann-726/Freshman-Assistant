...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    // 如果是新建，则弹出键盘
    if(_editState == kdairyEditeStateNew) {
...
- (void)keyboardDidHide:(NSNotification *)notif
{
    ...
    // 如果是编辑态，则右边为删除
    if(_editState == kdairyEditeStateEdit) {
...
-(void)buildUI
{
    ...
    // 如果是编辑态，则显示笔记的文字
    if(_editState == kdairyEditeStateEdit) {
        self.textView.text = _dairy.text;
        [self setNavRightButton: @"删除"];
    }
    // 初始状态下的键盘为不显示
    isKeyboardShow = NO;
...
- (void)rightBarButtonAction:(UIButton *)button
{
    if([[button titleForState: UIControlStateNormal] isEqualToString: @"完成"]) {
        ...
        if([self.textView hasText]) {
            ...
            if(_editState == kdairyEditeStateNew) {
                // 保存
                [_dairytool addNewDairy: self.textView.text];
                _dairy = [[_dairytool findAll] firstObject];
            } else {
                // 更新
                [_dairytool updateDairy: _dairy replaceString: self.textView.text];
            }
            
            // 完成后更新状态
            _editState = kdairyEditeStateEdit;
...
- (void)setNavRightButton:(NSString *)title
{
    if(title == nil) {
        ...
        if(!rightBarButton) {
            ...
            rightBarButton.backgroundColor=UIColorFromHEX(ColorCommonGreen);
